	...
	public static function save() {
		...
		if ( isset( $_POST['wfcm-settings'] ) ) {
			...
			foreach ( $wfcm_settings as $key => $value ) {
				...
				// if the scan frequency was changed then clear the scheduled
				// hook so it can add itself on the new time/schedule.
				if ( 'scan-frequency' === $key ) {
					$old_freq = wfcm_get_setting( 'scan-frequency' );
					$old_hour = wfcm_get_setting( 'scan-hour' );
					$old_day  = wfcm_get_setting( 'scan-day' );

					// Ensure changes are detected even if the "scan-day" option is hidden.
					$scan_day = ( $wfcm_settings['scan-frequency'] === 'weekly' ) ? $wfcm_settings['scan-day'] : 0;

					if ( $value !== $old_freq || $wfcm_settings['scan-hour'] !== $old_hour || $old_day !== $scan_day ) {
